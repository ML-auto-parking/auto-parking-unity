...
for _ in range(10):
    ...
    first_step = 3000000 + train_start_step
    ...
    save_path = f"./saved_models/{game}/DQN/a2-s2/ddqn-ver-{version}/{date_time}"
    load_path = f"./saved_models/{game}/DQN/a2-s2/ddqn-ver-{version}/06-10 05:55"
    # 20240602012700
    ...
    class DQNAgent:
        ...
        def train_model(self, beta=0.4):
            ...
            # Q(s', a') 계산
            with torch.no_grad():
                online_next_q_values = self.network(next_vectors, next_agent_pos)
                next_action = torch.argmax(online_next_q_values, dim=1, keepdim=True)
                target_q_values = self.target_network(next_vectors, next_agent_pos)
                max_target_q_values = target_q_values.gather(1, next_action)
                target_q = rewards + (1 - dones) * discount_factor * max_target_q_values
