...
for _ in range(10):
    ...
    first_step = 150000 + train_start_step
    ...
    save_path = f"./saved_models/{game}/DQN/a3-s2/ver-{version}/{date_time}"
    load_path = f"./saved_models/{game}/DQN/a3-s2/ver-{version}/06-05 08:17"
    # 20240530073302
    ...
    class DQNAgent:
        ...
        def get_action(self, vector, agent_pos, training=True):
            ...
            if epsilon > random.random():
                wheel_torque = np.random.randint(0, WHEEL_TORQUE_MAX, size=(vector.shape[0], 1))  # 휠 토크
                steering_angle = np.random.randint(0, STEERING_ANGLE_MAX, size=(vector.shape[0], 1))  # 조향 각도
        ...
        def train_model(self, beta=0.4):
            ...
            # one-hot 인코딩된 행동을 결합
            one_hot_action = torch.cat((wheel_torque_one_hot, steering_angle_one_hot, brake_state_one_hot),
                                       dim=1).float()
    ...
    if __name__ == '__main__':
        ...
        env = UnityEnvironment(file_name=env_name, side_channels=[engine_configuration_channel], worker_id=18)
        env.reset()
