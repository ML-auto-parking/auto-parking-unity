...
for _ in range(10):
    ...
    train_start_step = 500  # 초기 탐험
    first_step = 2000000 + train_start_step
    ...
    game = "AutoParking"
    version = "48"
    env_name = f'../Env/ap-ver-{version}'

    # 모델 저장 및 불러오기 경로
    date_time = datetime.datetime.now().strftime("%m-%d %H:%M")
    save_path = f"./saved_models/{game}/DQN/a2-s2/ver-{version}-2/{date_time}"
    load_path = f"./saved_models/{game}/DQN/a2-s2/ver-{version}-2/06-10 05:55"
    # 20240602012700
    ...
    class DQN(nn.Module):
        def __init__(self, image_dim, vector_dim, agent_pos_dim, action_dim):
            ...
            self.conv1 = torch.nn.Conv2d(in_channels=image_dim[0], out_channels=32, kernel_size=5, stride=2)
            self.conv2 = torch.nn.Conv2d(in_channels=32, out_channels=64, kernel_size=5, stride=2)
            self.conv3 = torch.nn.Conv2d(in_channels=64, out_channels=128, kernel_size=3, stride=1)
            self.conv4 = torch.nn.Conv2d(in_channels=128, out_channels=256, kernel_size=3, stride=1)

            def conv2d_size_out(size, kernel_size, stride):
                return (size - (kernel_size - 1) - 1) // stride + 1

            convw = conv2d_size_out(image_dim[1], 5, 2)
            convh = conv2d_size_out(image_dim[2], 5, 2)
            ...
            conv_output_size = convw * convh * 256
            linear_input_size = conv_output_size + vector_dim + agent_pos_dim

            self.fc1 = torch.nn.Linear(linear_input_size, 1024)
    ...
    class DQNAgent:
        ...
        def append_sample(self, image, vector, agent_pos, action, reward, next_image, next_vector, next_agent_pos, done):
            ...
            self.memory.append((image, vector, agent_pos, action_index, reward, next_image, next_vector, next_agent_pos, done))
    ...
    if __name__ == '__main__':
        ...
        agent = DQNAgent(image_dim, vector_size, needed_agent_pos_dim)
        ...
        for step in range(run_step + test_step):
            ...
            if len(decision_steps) > 0:  # 에이전트가 존재하는지 확인
                action = agent.get_action(image, vector, needed_agent_pos, train_mode)
            ...
            if train_mode:
                agent.append_sample(image, vector, needed_agent_pos, action, reward, next_image, next_vector, next_needed_agent_pos, [done])
