...
for _ in range(10):
    ...
    # 모델 저장 및 불러오기 경로
    date_time = datetime.datetime.now().strftime("%m-%d %H_%M")
    save_path = f"./saved_models/{game}/DQN/a2-s2/ver-{version}-3/{date_time}"
    load_path = f"./saved_models/{game}/DQN/a2-s2/ver-{version}-3/06-10 05_55"
    # 20240602012700
    ...
    class DQNAgent:
        def __init__(self, vector_dim, agent_pos_dim):
            ...
            self.optimizer = torch.optim.Adam(self.network.parameters(), lr=initial_learning_rate)
            lambda_lr = lambda step: max(min_learning_rate, initial_learning_rate * GAMMA_LR**(step//10000))
